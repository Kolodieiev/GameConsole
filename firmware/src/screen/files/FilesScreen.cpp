...
void FilesScreen::showFilesTmpl()
{
    ...
    Label *size_title_lbl = new Label(ID_SIZE_TITLE_LBL, _display);
    ...
    _file_size_lbl = new Label(ID_SIZE_LBL, _display);
    ...
    _file_pos_lbl = new Label(ID_FILE_POS_LBL, _display);
...
void FilesScreen::makePathFromBreadcrumbs(String &out_str)
{
    out_str = "";
...
void FilesScreen::showContextMenu()
{
    ...
    uint16_t id = _files_list->getCurrentItemID();
...
void FilesScreen::updateFileInfo()
{
    String file_name;
    makePathFromBreadcrumbs(file_name);
    file_name += "/";
    ...
    String pos = "[";
    pos += String(_files_list->getCurrentItemID());
    pos += "/";
...
void FilesScreen::fillFilesTmpl()
{
...
void FilesScreen::taskDone(bool result, void *arg)
{
    FilesScreen *this_ptr = static_cast<FilesScreen *>(arg);
    this_ptr->taskDoneHandler(result);
}

//-------------------------------------------------------------------------------------------

void FilesScreen::makeMenuFilesItems(std::vector<MenuItem *> &items, uint16_t file_pos, uint8_t size)
{
...
void FilesScreen::handleNextItemsLoad(std::vector<MenuItem *> &items, uint8_t size, uint16_t cur_id)
{
    if (!cur_id)
        return;
...
void FilesScreen::onNextItemsLoad(std::vector<MenuItem *> &items, uint8_t size, uint16_t cur_id, void *arg)
{
    FilesScreen *this_ptr = static_cast<FilesScreen *>(arg);
...
void FilesScreen::handlePrevItemsLoad(std::vector<MenuItem *> &items, uint8_t size, uint16_t cur_id)
{
    // Не отримали ID
    if (!cur_id)
        return;
...
void FilesScreen::onPrevItemsLoad(std::vector<MenuItem *> &items, uint8_t size, uint16_t cur_id, void *arg)
{
    FilesScreen *this_ptr = static_cast<FilesScreen *>(arg);
