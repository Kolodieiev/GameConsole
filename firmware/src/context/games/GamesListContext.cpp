...
GamesListContext::GamesListContext(GraphicsDriver &display) : IContext(display)
{
...
void GamesListContext::update()
{
    ...
    else if (_input.isReleased(KeyID::KEY_OK))
    {
        ...
        openContextByID((ContextID)_menu->getCurrentItemID());
    }
}
...
void GamesListContext::ok()
{
    ...
    if (screen_id == ID_CONTEXT_SNAKE)
        return; // Гру ще не реалізовано

    openContextByID((ContextID)screen_id); // Ігровий екран буде відкриватися без додаткової перевірки.
}
