...
{
    MainScene::MainScene(GraphicsDriver &display, Input &input, DataStream &stored_objs)
        : IGameScene(display, input, stored_objs), _server{stub_serv}, _client{stub_client}
    {
        _mode = MODE_SOLO;
        init();
    }

    MainScene::MainScene(GraphicsDriver &display, Input &input, DataStream &stored_objs, GameServer &server)
        : IGameScene(display, input, stored_objs), _server{server}, _client{stub_client}
    {
        _mode = MODE_SERVER;
        ...
        _server.sendBroadcast(UdpPacket::CMD_DATA, &START_GAME_DATA_CMD, 1);
    }

    MainScene::MainScene(GraphicsDriver &display, Input &input, DataStream &stored_objs, GameClient &client)
        : IGameScene(display, input, stored_objs), _server{stub_serv}, _client{client}
    {
        _mode = MODE_CLIENT;
    ...
    void MainScene::init()
    {
        ...
        if (_mode == MODE_SERVER || _mode == MODE_SOLO)
        {
            ...
            _main_obj = _ball;
            _game_objs.emplace(_ball->getObjId(), _ball);
        }

        buildMap();

        _ui = new SceneUI(_display);
        _game_UI = _ui;
        ...
        if (_mode == MODE_CLIENT) // Якщо клієнт, надсилаємо інформацію серверу про готовність приймати дані
        {
            ...
            _client.send(UdpPacket::CMD_DATA, &data, 1);
        }
    }
    ...
    {
        MainScene *this_ptr = static_cast<MainScene *>(arg);
        this_ptr->handleClientData(cl_wrap, packet);
    }
    ...
    void MainScene::doServerStuff()
    {
        ...
        packet.write((uint8_t)CMD_UPD_OBJ);
        serialize(packet);
    ...
    void MainScene::doClientStuff(IGameObject::MovingDirection direction)
    {
        ...
        pack.write((uint8_t)CMD_MOVE);
        pack.write((uint8_t)direction);
    ...
    void MainScene::handleClientData(ClientWrapper *cl_wrap, UdpPacket *packet)
    {
        ...
        // Зчитуємо саме ігрову команду, а не пакетну. Пакетну оброблює сервер самостійно.
        ...
        if (cmd == CMD_READY)
        {
            ...
            _game_objs.emplace(ball->getObjId(), ball);

            // Надіслати клієнту його id
            UdpPacket resp(SIZE_CMD_SET_ID); // Розмір команди в даних + розмір id
            resp.write((uint8_t)CMD_SET_ID);
            resp.write(ball->getObjId());
    ...
    void MainScene::handleServerData(UdpPacket *packet)
    {
        ...
        else if (cmd == CMD_INIT)
        {
            ...
            while (packet->space() > 5) // В роділі даних повинно зберігатися мінімум (obj_id(4) + class_id(1) + 1 ) для відновлення
            {
                ...
                if (obj->getObjId() == _client_obj_id)
                {
                ...
                _game_objs.emplace(obj->getObjId(), obj);
            }
        }
