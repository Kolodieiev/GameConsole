...
void TFT_eSPI::pushBlock(uint16_t color, uint32_t len)
{
  ...
  if (rem)
  {
    ...
    *_spi_cmd = SPI_USR;
    if (!len)
      return; //{while (*_spi_cmd&SPI_USR); return; }
    i = i >> 1;
    while (i++ < 16)
      *spi_w++ = color32;
  }
...
void TFT_eSPI::pushPixelsDMA(uint16_t *image, uint32_t len)
{
  ...
  while (len > 0x4000)
  { // Transfer 16 bit pixels in blocks if len*2 over 65536 bytes
    pushPixels(image, 0x400);
    len -= 0x400;
    image += 0x400; // Arbitrarily send 1K pixel blocks (2Kbytes)
  }
...
void TFT_eSPI::pushImageDMA(int32_t x, int32_t y, int32_t w, int32_t h, uint16_t const *image)
{
  ...
  { // Transfer 16 bit pixels in blocks if len*2 over 65536 bytes
    ...
    len -= 0x400;
    buffer += 0x400; // Arbitrarily send 1K pixel blocks (2Kbytes)
  }
...
#endif //defined(CONFIG_IDF_TARGET_ESP32S3) or defined(CONFIG_IDF_TARGET_ESP32S2)
